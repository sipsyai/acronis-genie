---
title: "Start test failover for a specific recovery server"
source: "https://developer.acronis.com/doc/outbound/apis/api-library/dr/cloud-servers/start-test-failover.html"
source_domain: "developer"
scraped_at: "2026-03-22T02:21:37.057383"
---
...
...             start_test_failover_link = i['href']
...             break
...
>>> if start_test_failover_link is None:
...     print('Cannot start test failover.')


If there are no issues and the link is present, the start_test_failover_link variable will contain the URL to the Disaster Recovery API
endpoint that you may use to start the test failover.
...
Send a POST request to the Disaster Recovery API using the start_test_failover_link URL and the scoped authentication token:
>>> response = requests.post(start_test_failover_link, headers=scoped_auth, data=recovery_point_data)
...
Status code 202 means that the request was successful and the test failover has been initiated.
