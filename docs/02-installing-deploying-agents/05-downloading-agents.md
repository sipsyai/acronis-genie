---
title: "Downloading protection agents"
...
3. In the **Add devices** panel, from the **Release channel** drop-down menu, select an agent version.
   - **Latest** - download the latest available agent version.
   - **Previous stable** - download the agent version from the previous release.
4. Select the agent that corresponds to the operating system of the workload that you are adding. The **Save As** dialog opens.
5. [Only for Macs with Apple silicon processors, such as M1 and M2] Click **Cancel**. In the **Add Mac** panel that opens, click **Download ARM installer**.
